...
function PagesManager() {
    ...
    this.changePage = function(
        pageName,
        { pushToHistory = true, query = false, path = false } = {}
    ) {
        ...
        if (!_this.pages[pageName]) {
            ...
            _this.pages[
                pageName
            ].container = elements.pagesContainer.addElement("div", {
                class: `pageContainer ${pageName}PageContainer none`
            });
        }
        ...
        stylesheets.forEach(stylesheet => {
            let cssLink = document.head.addElement("link", {
                rel: "stylesheet",
                href: stylesheet
            });
        ...
        //title
        var documentTitle =
            config.pageTitlePrefix +
            (pageConfig.title || pageName) +
        ...
        if (this.pages[pageName].isLoaded) {
            if (pageConfig.refreshDataOnDisplay) {
                //reload data
    ...
    this.manageData = function(pageName) {
        ...
        if (
            !Array.isArray(allDataConfigs) &&
            typeof allDataConfigs === "object"
        ) {
            allDataConfigs = [allDataConfigs];
        }
        for (
            var indDataConfig = 0;
            indDataConfig < allDataConfigs.length;
            indDataConfig++
        ) {
    ...
    this.applyDataConfig = function(dataConfig, pageName) {
        ...
        if (adapters[dataConfig.adapter]) {
            ...
            adaptersContainer = _this.pages[pageName].container.querySelector(
                dataConfig.container
            );
            if (!adaptersContainer) {
                console.warn(
                    `data container for ${dataConfig.container} selector, doesn't exist`
                );
                return;
            }
        ...
        if (dataConfig.pathTemplate) {
            ...
            for (let indPath = 0; indPath < pathEntities.length; indPath++) {
                ...
                if (
                    !templateEntity.includes("{{") ||
                    !templateEntity.includes("}}")
                ) {
                    //template doesn't correspond to url
                    if (templateEntity !== paramValue) {
                        console.warn(
                            "url path doesnt apply to template. aborting.",
                            { templateEntity, paramValue }
                        );
                        return;
                    }
                    continue;
        ...
        dataSource(dataParams).then(function(data) {
            if (adapter) {
                //adapter data
    ...
    this.loadView = async function(view, callBack = () => {}) {
        ...
        if (!response.ok) {
            ...
            for (
                var indEvt = 0;
                indEvt < viewsCache[viewName].onload.length;
                indEvt++
            ) {
                viewsCache[viewName].onload[indEvt]({
                    clientMsg: "view download failed"
                });
            }
